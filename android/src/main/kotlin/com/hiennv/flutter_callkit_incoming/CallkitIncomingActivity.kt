...
class CallkitIncomingActivity : Activity() {
    ...
    private fun onAcceptClick() {
        Log.d("this is accept", "1")
        val data = intent.extras?.getBundle(EXTRA_CALLKIT_INCOMING_DATA)
        Log.d("this is accept", "2")
        val intent = packageManager.getLaunchIntentForPackage(packageName)?.cloneFilter()
        Log.d("this is accept", "3")
        if (isTaskRoot) {
            Log.d("this is accept", "4")
            intent?.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK)
        } else {
            Log.d("this is accept", "5")
            intent?.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_CLEAR_TOP)
        }
        if (intent != null) {
            Log.d("this is accept", "6")
